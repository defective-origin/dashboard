...
export function MarkupBoardAddLine(props: MarkupBoardAddLineProps) {
  ...
  return (
    <MarkupBoardAction
      ...
      icon='health_cross'
      actions={size => [
        { content: t('LABEL.ROW'), start: 'splitscreen_add', color: 'success', onClick: () => props.row && onRowAdd?.(props.row, size) },
        { content: t('LABEL.COLUMN'), start: 'splitscreen_vertical_add', color: 'warning', onClick: () => props.column && onColumnAdd?.(props.column, size) },
      ]}
      {...otherProps}
    />
  )
