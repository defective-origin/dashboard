export const toCoordinate = (row1: number, column1: number, row2: number, column2: number) => ({
...
export const initMatrix = <T = any>(rows: number, columns: number, fill: (row: number, column: number) => T) => {
  return Array.from({ length: rows }, (_, row) => Array.from({ length: columns }, (_, column) => fill(row, column)))
}

export const toString = <T = any>(matrix: T[][], sep = ', ') => {
...
export const selectRowsMatch = <T>(matrix: T[][], index1: number, index2: number, fill?: any) => {
  checkMatrix(matrix)
...
/** return line with matched items in both lines or fill if not matching */
export const selectColumnsMatch = <T>(matrix: T[][], index1: number, index2: number, fill?: any) => {
  checkMatrix(matrix)
...
export const insertColumn = <T>(matrix: T[][], index: number, column: T[]) => {
  checkMatrix(matrix)
  checkMatrixLine(matrix[0].length, column.length)
...
export const extendRows = <T>(matrix: T[][], index: number, empty: T) => {
  ...
  const line = selectRowsMatch(matrix, index, index + 1, empty)

  console.warn(line, matrix)

  return insertRow(matrix, index, line)
}
...
export const extendColumns = <T>(matrix: T[][], index: number, empty: T) => {
  ...
  const line = selectColumnsMatch(matrix, index, index + 1, empty)
