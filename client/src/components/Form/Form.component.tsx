...
import { useEvent, useFunc, useSubscriptions } from 'hooks'
...
export type FormProps<O extends object = object> = Pick<LayoutProps, 'p' | 'g' | 'v'> & {
...
export function Form<O extends object>(props: FormProps<O>): JSX.Element {
  const { init, resetId = '', submitId = '', manager, checkOnSubmit, saveOnSubmit, notifyParent, onSubmit, onChange, onReset, p, g = 'xs', v, children, className } = props
  ...
  useEvent('click', () => submit(), { ref: () => document.getElementById(submitId) })
  useEvent('click', () => reset(), { ref: () => document.getElementById(resetId) })
