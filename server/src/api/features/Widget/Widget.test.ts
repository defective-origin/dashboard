...
import { Widget, WidgetModel, PATHNAME } from './Widget.model'
...
describe(`[${PATHNAME}] api`, () => {
  beforeEach(async () => {
    savedRecord = await WidgetModel.create(MOCK_SAVED_RECORD)
  })
  ...
  describe('[create] endpoint', () => {
    it('should create new record', async () => {
      ...
      const record = await WidgetModel.findOne({ name: MOCK_OPERATION_RECORD.name })
  ...
  describe('[delete] endpoint', () => {
    it('should delete record by id', async () => {
      ...
      const record = await WidgetModel.findOne(MOCK_SAVED_RECORD)
